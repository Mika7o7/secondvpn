...
async def extend_subscription(event: types.Message | types.CallbackQuery, state: FSMContext):
    ...
    text = (
        f"💾 Продление доступа к Zion:\n{price_list}\n\n"
        f"Или введи любое количество месяцев.\n"
        f"Цена вне акции — {STANDARD_PRICE_PER_MONTH} руб./мес.\n\n"
        f"Сколько месяцев ты хочешь продлить доступ?"
    )
...
async def process_months(message: types.Message, state: FSMContext):
    ...
    try:
        months = int(text)
        if months <= 0:
            await message.answer("Пожалуйста, введи положительное число месяцев.")
            return
        ...
        amount = DISCOUNTED_PRICES.get(months, months * STANDARD_PRICE_PER_MONTH)
...
async def ignore_commands(message: types.Message):
    await message.answer("Пожалуйста, введите количество месяцев.")
...
async def confirm_payment(call: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        logger.info("▶️ Получено %d транзакций от CloudTips", len(items))

        for item in items:
            comment = (item.get("comment") or "").strip().casefold()
            token_check = payment_token.strip().casefold()

            logger.info("🔎 Проверка транзакции: comment=%r | token=%r", comment, token_check)
            ...
            if comment == token_check:
                ...
                try:
                    ...
                    logger.error("❌ Ошибка продления доступа: %s", str(e))
        ...
        logger.error("❌ Ошибка в confirm_payment: %s", str(e))
